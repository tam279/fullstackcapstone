CREATE DATABASE IF NOT EXISTS PMWDATABASE CHARACTER SET UTF8MB4 COLLATE UTF8MB4_UNICODE_CI;

USE PMWDATABASE;

-- Drop tables if they exist
SET FOREIGN_KEY_CHECKS = 1;

DROP TABLE IF EXISTS PASSWORD_RESET_TOKENS;

DROP TABLE IF EXISTS USER_ACTIVITY;

DROP TABLE IF EXISTS COMMENT_FILE_BRIDGE;

DROP TABLE IF EXISTS TASK_USER_BRIDGE;

DROP TABLE IF EXISTS TASK_TECHNICIAN_BRIDGE;

DROP TABLE IF EXISTS VIEWER_BRIDGE;

DROP TABLE IF EXISTS PROJECTTAGBRIDGE;

DROP TABLE IF EXISTS USERTAGBRIDGE;

DROP TABLE IF EXISTS TASK_FILE_BRIDGE;

DROP TABLE IF EXISTS DEPENDENCY_BRIDGE;

DROP TABLE IF EXISTS COMMENT;

DROP TABLE IF EXISTS FILE;

DROP TABLE IF EXISTS TASK;

DROP TABLE IF EXISTS PROJECT;

DROP TABLE IF EXISTS TAG;

DROP TABLE IF EXISTS LOGIN_METHOD;

DROP TABLE IF EXISTS USER;

DROP TABLE IF EXISTS ROLE;

DROP TABLE IF EXISTS COMPANY;

SET FOREIGN_KEY_CHECKS = 1;

-- Create tables

CREATE TABLE COMPANY (
    COMPANYID INT AUTO_INCREMENT PRIMARY KEY,
    COMPANYNAME VARCHAR(255),
    CREATED_AT TIMESTAMP NULL DEFAULT NULL,
    ISACTIVE INT DEFAULT 1
);

CREATE TABLE ROLE (
    ROLEID INT AUTO_INCREMENT PRIMARY KEY,
    ROLENAME VARCHAR(255)
);

CREATE TABLE LOGIN_METHOD (
    METHODID INT AUTO_INCREMENT PRIMARY KEY,
    METHODNAME VARCHAR(255)
);

CREATE TABLE USER (
    EMAIL VARCHAR(255) PRIMARY KEY,
    PASSWORD VARCHAR(255),
    FIRSTNAME VARCHAR(255),
    LASTNAME VARCHAR(255),
    ISACTIVE INT DEFAULT 1,
    IS_EMAIL_VERIFIED INT DEFAULT 0,
    TAG VARCHAR(255),
    COMPANYID INT,
    ROLEID INT,
    METHODID INT,
    UNIQUE(EMAIL),
    FOREIGN KEY(COMPANYID) REFERENCES COMPANY(COMPANYID),
    FOREIGN KEY(ROLEID) REFERENCES ROLE(ROLEID),
    FOREIGN KEY(METHODID) REFERENCES LOGIN_METHOD(METHODID)
);

CREATE TABLE USER_ACTIVITY (
    ACTIVITYID INT AUTO_INCREMENT PRIMARY KEY,
    EMAIL VARCHAR(255),
    TIMESTAMP TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    DESCRIPTION VARCHAR(255),
    FOREIGN KEY(EMAIL) REFERENCES USER(EMAIL)
);

CREATE TABLE PROJECT (
    PROJECTID INT AUTO_INCREMENT PRIMARY KEY,
    NAME VARCHAR(255),
    STARTDATE DATE,
    ENDDATE DATE,
    PROGRESS INT DEFAULT 0,
    MANAGEREMAIL VARCHAR(255),
    DESCRIPTION VARCHAR(255),
    COMPANYID INT,
    ISACTIVE INT DEFAULT 1,
    FOREIGN KEY(MANAGEREMAIL) REFERENCES USER(EMAIL),
    FOREIGN KEY(COMPANYID) REFERENCES COMPANY(COMPANYID)
);

CREATE TABLE VIEWER_BRIDGE (
    EMAIL VARCHAR(255),
    PROJECTID INT,
    PRIMARY KEY(EMAIL, PROJECTID),
    FOREIGN KEY(EMAIL) REFERENCES USER(EMAIL),
    FOREIGN KEY(PROJECTID) REFERENCES PROJECT(PROJECTID)
);

CREATE TABLE TASK (
    TASKID INT AUTO_INCREMENT PRIMARY KEY,
    NAME VARCHAR(255),
    STARTDATE DATE,
    ENDDATE DATE,
    PROGRESS INT DEFAULT 0,
    DESCRIPTION VARCHAR(255),
    STATUS VARCHAR(255),
    PRIORITY VARCHAR(255),
    ISACTIVE INT DEFAULT 1,
    PROJECTID INT,
    FOREIGN KEY(PROJECTID) REFERENCES PROJECT(PROJECTID)
);

CREATE TABLE TASK_USER_BRIDGE (
    EMAIL VARCHAR(255),
    TASKID INT,
    PRIMARY KEY(EMAIL, TASKID),
    FOREIGN KEY(EMAIL) REFERENCES USER(EMAIL),
    FOREIGN KEY(TASKID) REFERENCES TASK(TASKID)
);

CREATE TABLE TASK_TECHNICIAN_BRIDGE (
    EMAIL VARCHAR(255),
    TASKID INT,
    PRIMARY KEY(EMAIL, TASKID),
    FOREIGN KEY(EMAIL) REFERENCES USER(EMAIL),
    FOREIGN KEY(TASKID) REFERENCES TASK(TASKID)
);

CREATE TABLE TAG (
    TAGID INT AUTO_INCREMENT PRIMARY KEY,
    TAGNAME VARCHAR(255)
);

CREATE TABLE USERTAGBRIDGE (
    TAGID INT,
    EMAIL VARCHAR(255),
    PRIMARY KEY(TAGID, EMAIL),
    FOREIGN KEY(TAGID) REFERENCES TAG(TAGID),
    FOREIGN KEY(EMAIL) REFERENCES USER(EMAIL)
);

CREATE TABLE PROJECTTAGBRIDGE (
    TAGID INT,
    PROJECTID INT,
    PRIMARY KEY(TAGID, PROJECTID),
    FOREIGN KEY(TAGID) REFERENCES TAG(TAGID),
    FOREIGN KEY(PROJECTID) REFERENCES PROJECT(PROJECTID)
);

CREATE TABLE COMMENT (
    COMMENTID INT AUTO_INCREMENT PRIMARY KEY,
    COMMENT VARCHAR(255),
    DATE DATE,
    TASKID INT,
    EMAIL VARCHAR(255),
    FOREIGN KEY(TASKID) REFERENCES TASK(TASKID),
    FOREIGN KEY(EMAIL) REFERENCES USER(EMAIL)
);

CREATE TABLE FILE (
    FILEID INT AUTO_INCREMENT PRIMARY KEY,
    FILENAME VARCHAR(255),
    FILELOCATION VARCHAR(255)
);

CREATE TABLE COMMENT_FILE_BRIDGE (
    COMMENTID INT,
    FILEID INT,
    PRIMARY KEY(COMMENTID, FILEID),
    FOREIGN KEY(COMMENTID) REFERENCES COMMENT(COMMENTID),
    FOREIGN KEY(FILEID) REFERENCES FILE(FILEID)
);

CREATE TABLE TASK_FILE_BRIDGE (
    FILEID INT,
    TASKID INT,
    PRIMARY KEY(FILEID, TASKID),
    FOREIGN KEY(FILEID) REFERENCES FILE(FILEID),
    FOREIGN KEY(TASKID) REFERENCES TASK(TASKID)
);

CREATE TABLE DEPENDENCY_BRIDGE (
    TASKID INT,
    DEPENDSON_TASKID INT,
    PRIMARY KEY(TASKID, DEPENDSON_TASKID),
    FOREIGN KEY(TASKID) REFERENCES TASK(TASKID),
    FOREIGN KEY(DEPENDSON_TASKID) REFERENCES TASK(TASKID)
);

CREATE TABLE PASSWORD_RESET_TOKENS (
    EMAIL VARCHAR(255),
    TOKEN VARCHAR(255),
    EXPIRY_DATE TIMESTAMP NULL DEFAULT NULL,
    PRIMARY KEY(TOKEN),
    FOREIGN KEY(EMAIL) REFERENCES USER(EMAIL)
);